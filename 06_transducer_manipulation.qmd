...
!echo "ночка" | hfst-lookup examples/analyzer.hfstol
```
...
```{{shell}}
hfst-intersect generator.hfst generator2.hfst | hfst-fst2strings
```
```{python}
!hfst-intersect examples/generator.hfst examples/generator2.hfst | hfst-fst2strings
```
...
```{{shell}}
hfst-union generator.hfst generator2.hfst | hfst-fst2strings
```
```{python}
!hfst-union examples/generator.hfst examples/generator2.hfst | hfst-fst2strings
```

Потенциально это важный инструмент, позволяющий разделить работу над разными фрагментами языка. Для каких-то отдельных словоизменительных типов (частей речи, или лексем с дефектными парадигмами) можно делать отдельные `.lexd` и `.twol` модули, которые потом соединяются во-едино, без опасности что-то поломать в общем `.twol`-файле.
